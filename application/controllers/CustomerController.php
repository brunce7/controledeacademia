...
class CustomerController extends Zend_Controller_Action {
    ...
    public function createAction() {
        ...
        if ($this->_request->isPost()) {
            if ($form->isValid($dadosFormulario)) {
                ...
                $monthly_payment = str_replace(",", ".", $valor1);
                ...
                if ($cadastro->gravar($data)) {
                    ...
                    $this->view->resp = "Aluno(a):  " . $name . ",  foi cadastrado(a) com sucesso!";
                    ...
                    $this->view->resp = "Aluno(a):  " . $name . "Nao pode ser cadastrado!";
                    $resp1 = $this->view->layout()->scriptTags = '<script>
                        $(function(){
                           $("#dialog").dialog({
                              modal: open
        ...
        if ($this->_request->isPost()) {
            ...
            if ($form->isValid($dadosFormulario)) {
                ...
                if ($customerModel->editar($data, $id)) {
                    ...
                    $this->view->resp = "Aluno(a):  " . $name . ", não pode ser atualizado(a)!<br>Voce deve 1º alterar um campo.";
                    $resp1 = $this->view->layout()->scriptTags = '<script>
                        $(function(){
                           $("#dialog").dialog({
                              modal: open
    ...
    public function buscarAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $lista = $alunos->buscar($id, $name, $option1, $option2, $option3);
            if (count($lista)) {
                $this->view->listaAlunos = $lista;
                $resp = "Sucesso!";
                $alerta = "text-success";
    ...
    public function deletarAction() {
        ...
        if ($this->getRequest()->getParam("excluir")) {
            ...
            if ($customerModel->deletar($del)) {
                ...
                $this->view->titulo = "Sucesso!";
                // $this->view->form = $form;
                $this->view->resp = "Aluno:  " . $name . ",  foi excluido com sucesso!";
                //$resp1 = $this->view->layout()->scriptTags = '<script>
                //   $(function(){
                //     $("#dialog").dialog({
                //     modal: open
                ...
                $this->view->resp = "Aluno:  " . $name . ",  não pode ser excluido!";
                //   $resp1 = $this->view->layout()->scriptTags = '<script>
                //     $(function(){
                //       $("#dialog").dialog({
                //          modal: open
    ...
    public function paymentAction() {
        ...
        $customerModel = new Application_Model_DbTable_Customer();
        $alunos = new Application_Model_DbTable_Payment();
        ...
        $form->getElement('description')->setAttrib('required', 'required');
         $form->getElement('customer')->setAttrib('placeholder', 'Busca pela Matrícula');
          $form->getElement('name')->setAttrib('placeholder', 'Busca pelo Nome do Aluno');
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($dadosFormulario)) {
                ...
                if ($dbPayment->gravaPayment($data)) {
                    ...
                    $this->view->resp = "O Pagamento do Aluno(a): " . $nome . ", foi registrado com sucesso!";
                    $resp1 = $this->view->layout()->scriptTags = '<script>
                        $(function(){
                           $("#dialog").dialog({
                              modal: open
                    ...
                    $this->view->resp = "O Pagamento do Aluno(a): " . $nome . ", Nao pode ser registrado!";
                    $resp1 = $this->view->layout()->scriptTags = '<script>
                        $(function(){
                           $("#dialog").dialog({
                              modal: open
